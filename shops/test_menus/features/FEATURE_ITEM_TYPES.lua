...
local function getTestItemsMenus()
    local menuTypes = {
        { Id = "BUSINESS",  Label = "Business Item",  Description = "Business items are primarily used in the moonshine shack." },
        { Id = "COUPON",    Label = "Coupon Item",    Description = "Coupon items are used to display coupons providing discounts on purchases." },
        { Id = "HAIR",      Label = "Hair Item",      Description = "Hair items are used at barbers to display hairstyles and facial hair." },
        { Id = "INVENTORY", Label = "Inventory Item", Description = "Inventory items are used throughout the game for various purposes." },
        { Id = "PALETTE",   Label = "Palette Item",   Description = "Palette items are used to display chosen colors for clothing and other items." },
        { Id = "STABLE",    Label = "Stable Item",    Description = "Stable items are used to display horse-related items." },
        { Id = "STEPPER",   Label = "Stepper Item",   Description = "Stepper items are used to select options from a list." },
        { Id = "TEXT",      Label = "Text Item",      Description = "Text items are used throughout the game to display various text elements." },
    }

    local menus = {}

    for _, item in ipairs(menuTypes) do
        local items = {}

        for i = 1, 10 do
            local newItem = {
                Id = "TEST_ITEMS_" .. item.Id .. "_ITEM_" .. i,
                Type = item.Id,
                Label = item.Label .. " " .. i,
                Data = getTestItemsItems(item.Id)
            }

            if item.Id == "STEPPER" then
